...
desc = [];
pos = pos((scale(:, 3)-1)<0.000001, :);
deleteIndex = [];
c=0;
for i=1:length(pos)
    imageSize = size(im);
    tmp = zeros(1,121);
    centerRow = floor(pos(i,2));
    centerCol = floor(pos(i,1));
    centerInten = im(centerRow, centerCol);
    
    flag = true;
    for rowOffset=-5:5
        for colOffset=-5:5
            nowRow = centerRow+rowOffset;
            nowCol = centerCol+colOffset;
            if nowRow>0 && nowRow<=imageSize(1) && nowCol>0 && nowCol<=imageSize(2)
                tmp((rowOffset+5)*11 + (colOffset+6)) = (im(nowRow, nowCol) >= centerInten);
                %display(nowCol);
            else
                flag = false;
            end
